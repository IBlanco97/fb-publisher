 ...
 *
 * Every branch resolves through an is_active-filtered query (or is filtered
 * against one), so a group toggled "Inactivo" in the dashboard is never a
 * valid target here — the scheduler doesn't need a separate isActive check
 * once a rule reaches executeRule.
 */
 ...
 * `dryRun: true` swaps every account's publisher for DryRunPublisher — no
 * browser, no Facebook, just a logged simulation of what would've posted.
 */
 ...
 * the CLI alike. `dryRun` runs the exact same cron/jitter/rate-limit
 * pipeline but never launches a browser or touches Facebook.
 */
export function startScheduler(opts: { headless?: boolean; dryRun?: boolean } = {}) {
  ...
  const publishers = buildPublishersByAccount(headless, opts.dryRun);
  const rules = scheduleRepo.getActive();

  emit(
    'info',
    `Arrancando scheduler — navegador ${headless ? 'oculto (headless)' : 'VISIBLE'}${opts.dryRun ? ' [DRY RUN]' : ''}`
  );
...
async function executeRule(
  ...
  if (rule.templateIds.length === 0) {
    const reason = 'Sin plantillas configuradas';
  ...
  if (targetGroupIds.length === 0) {
    const reason = 'El objetivo de la regla no resuelve a ningún grupo activo';
    emit('warn', reason, tag);
    return { outcome: 'skipped', reason };
  }
 ...
 * `dryRun: true` (default false here) skips the real Playwright publisher
 * entirely — same rule, same checks, just simulated instead of posted.
 */
export async function triggerRuleNow(
  ruleId: string,
  opts: { headless?: boolean; force?: boolean; dryRun?: boolean } = {}
): Promise<ExecutionOutcome> {
  const rule = scheduleRepo.getById(ruleId);
  if (!rule) {
  ...
  emit(
    'info',
    `Ejecución manual solicitada — navegador ${headless ? 'oculto' : 'VISIBLE'}${opts.force ? ', ignorando espaciados' : ''}${opts.dryRun ? ' [DRY RUN]' : ''}`,
    rule
  );
